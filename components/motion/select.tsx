...
interface SelectContextValue {
  ...
  registerNode: (value: string, el: HTMLButtonElement | null) => void;
  focusSelection: () => void;
  optionRefs: React.MutableRefObject<Map<string, HTMLButtonElement>>;
}
...
export function Select({
  ...
  const registerNode = useCallback((v: string, el: HTMLButtonElement | null) => {
    if (el) optionRefs.current.set(v, el);
    else optionRefs.current.delete(v);
  }, []);

  const focusSelection = useCallback(() => {
    const options = current !== undefined ? optionRefs.current.get(current) : undefined;
    (options ?? optionRefs.current.values().next().value)?.focus();
  }, [current]);
  ...
  return (
    <motion.button
      ...
      onKeyDown={(event) => {
        ...
        window.setTimeout(ctx.focusSelection, 0)
      }}
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  const onListKeyDown = (event: React.KeyboardEvent) => {
    if (event.key !== "ArrowDown" && event.key !== "ArrowUp" && event.key !== "Home" && event.key !== "End")
      return;
    const options = ctx.optionRefs.current;
    if (options.size === 0) return;
    const values = [...options.keys()];
    const activeValue = [...options].find(([, el]) => el === document.activeElement)?.[0];
    let index = activeValue ? values.indexOf(activeValue) : (ctx.value !== undefined ? values.indexOf(ctx.value) : 0);
    if (index === -1) index = 0;
    let target = index;
    if (event.key === "ArrowDown") target = (index + 1) % values.length;
    else if (event.key === "ArrowUp") target = (index - 1 + values.length) % values.length;
    else if (event.key === "Home") target = 0;
    else target = values.length - 1;
    event.preventDefault();
    options.get(values[target])?.focus();
  };
...
export interface SelectItemProps {
  value: string;
  /** Short text used in the closed trigger. Falls back to `children`. */
  label?: string;
  disabled?: boolean;
  className?: string;
  children: ReactNode;
...
export function SelectItem({
  ...
  useLayoutEffect(() => {
    ctx.registerNode(value, disabled ? null : optionRef.current);
    return () => ctx.registerNode(value, null);
  }, [ctx.registerNode, value, disabled]);
